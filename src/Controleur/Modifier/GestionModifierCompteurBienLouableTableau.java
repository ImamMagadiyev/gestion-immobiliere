...
import Modele.dao.requetes.Select.RequeteSelectAllByNumeroFiscaleCompteur;
import Vue.Modification.FenetreModifierCompteurBienLouable;
import Vue.Modification.FenetreModifierCompteurBienLouableTableau;

public class GestionModifierCompteurBienLouableTableau implements ActionListener {

    private FenetreModifierCompteurBienLouableTableau vue;
    private DaoCompteur dao = new DaoCompteur();

    public GestionModifierCompteurBienLouableTableau(FenetreModifierCompteurBienLouableTableau vue) {
        this.vue = vue;
        ...
        System.out.println("[DEBUG] Initialisation GestionModifierCompteurBienLouableTableau pour le bien : " + vue.getBien().getNumero_fiscale());
        chargerCompteurs(); 
    }
    ...
    private void chargerCompteurs() {
        try {
            ...
            List<Compteur> liste = dao.find(
                new RequeteSelectAllByNumeroFiscaleCompteur(),
                vue.getBien().getNumero_fiscale()
            );

            System.out.println("[DEBUG] " + liste.size() + " compteurs chargés pour le bien " + vue.getBien().getNumero_fiscale());
    ...
    private void modifier() {
        ...
        try {
            ...
            FenetreModifierCompteurBienLouable f =
                new FenetreModifierCompteurBienLouable(
                    c,
                    this::chargerCompteurs,
                    vue.getDesktop()
                );

            vue.getDesktop().add(f);
            f.setVisible(true);
